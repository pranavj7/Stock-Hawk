    ...
    if (urlStringBuilder != null){
      ...
      try{
        ...
        try {
          ...
          ArrayList array = Utils.quoteJsonToContentVals(getResponse);
          if (array.size() == 0) {
            String symbol = params.getExtras().getString("symbol");
            // publish a QueryEvent to subscribers
            EventBus.getDefault().post(new DisplayToast("Invalid stock symbol \"" + symbol + "\""));
          }
          else
            mContext.getContentResolver().applyBatch(QuoteProvider.AUTHORITY, array);
